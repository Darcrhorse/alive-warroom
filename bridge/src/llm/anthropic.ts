...
export class AnthropicClient implements LLMClient {
  ...
  async getDecision(gameState: GameState, history: EventHistory): Promise<GMDecision> {
    ...
    return this.getDecisionWithPrompt(prompt);
  }

  /**
   * Get a Game Master decision using a custom prompt
   * This allows for more flexible prompting when the default prompt doesn't fit
   */
  async getDecisionWithPrompt(prompt: string): Promise<GMDecision> {
    logger.info('Requesting decision from Anthropic', { model: this.model });
    ...
    try {
      ...
      logger.error('Error getting decision from Anthropic', { error });
      throw error;
    }
  }
  ...
  async generateSQF(action: GMAction, context: GameContext): Promise<string> {
    ...
    logger.info('Requesting SQF generation from Anthropic', { actionType: action.type });
    ...
    try {
      ...
      logger.error('Error generating SQF from Anthropic', { error });
      throw error;
    }
  }
